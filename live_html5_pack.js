...
var CommentManager = (function() {
	...
	function CommentManager(stageObject){
		...
		ticking=false;;
		this.canvasDrawerWrapper = function(){
			if(ticking)return;
			ticking=true;
			if(prevOpacity != self.options.global.opacity){
				prevOpacity = self.options.global.opacity;
				self.canvas.style.opacity=prevOpacity;
			}
			self.canvasDrawer();
			ticking=false;
		};
	...
	CommentManager.prototype.canvasResize = function(){
		...
		canvas=this.canvas,canvasData=canvas.getContext('2d').getImageData(0, 0, canvas.width, canvas.height);
	...
	CommentManager.prototype.canvasDrawer = function(){
		...
		var now = Date.now(), cmt, i, devicePixelRatio = window.devicePixelRatio, pausedTime = this.pausedTime,
		ctx = this.canvasOffscreen.getContext('2d'),
		...
		this.runline.forEach(function(cmt){
			...
			switch(cmt.mode){
				case 1:
					...
					x = (canvasWidth - cmt.rx) * devicePixelRatio;
					y = cmt.y * devicePixelRatio;
					cmt.prev = now;
				break;
				case 4:
					//bottom
					cmt.x = (canvasWidth - cmt.width) / 2;
					x = cmt.x * devicePixelRatio;
					y = (canvasHeight - cmt.y - cmt.height) * devicePixelRatio;
				break;
				case 5:
					//top
					cmt.x = (canvasWidth - cmt.width) / 2;
			...
			ctx.drawImage(cmt.textData, round(x), round(y));
		});
		
		//this.canvas.style.opacity = this.options.global.opacity;
		var cctx=this.canvas.getContext('2d')
		cctx.clearRect(0,0,this.canvas.width,this.canvas.height);
		cctx.drawImage(this.canvasOffscreen,0,0);
	...
	CommentManager.prototype.sendCanvas = function(data){
		if(!data)
			return;
		var now = Date.now();
...
(function() {
	...
	versionString='HTML5 Player ver.170303 based on ABPlayer-bilibili-ver',
	$ = function(e) {
	...
	ABP.bind = function(playerUnit, state) {
		...
		renderColumn=function(column,arr){
			var max=0,i,points=[];
			arr.find(function(i){max=(i>max)?i:max});
			max=max||1;
			for(i in arr){
		...
		setInterval(function(){
			...
			if(player.flv!=null){
				...
				var i=0,mediaInfo=player.flv._mediaInfo,statisticsInfo=player.flv.statisticsInfo;
				/*['mimeType','audioCodec','videoCodec'].find(function(name){
					flvjsStats[i++].innerHTML=mediaInfo[name];
				})*/
				flvjsStats[i++].innerHTML=mediaInfo.videoCodec;
				flvjsStats[i++].innerHTML=mediaInfo.audioCodec;
			...
			if(enabledStats.gecko){
				['mozParsedFrames','mozDecodedFrames','mozPaintedFrames'].find(function(name){
					document_querySelector('#'+name).innerHTML=video[name];
				})
			}
			
			if(enabledStats.webkit){
				['webkitDecodedFrameCount','webkitDroppedFrameCount'].find(function(name){
					document_querySelector('#'+name).innerHTML=video[name];
				})
			}
		...
		if (video.isBound !== true) {
			...
			ABPInst.videoDiv.parentNode[addEventListener]('touchstart',function(e){
				...
				var box,dmList=[],i=0,len=ABPInst.divComment.childNodes.length,x,y;
				for(;i<len;i++){
					box=ABPInst.divComment.childNodes[i].getBoundingClientRect();
					x=e.touches[0].clientX-box.left;
					y=e.touches[0].clientY-box.top;
					if(x>=0&&x<=box.width &&
						 y>=0&&y<=box.height)
						dmList.push(ABPInst.divComment.childNodes[i]);
				}
			...
			ABPInst.videoDiv.parentNode[addEventListener]('touchmove',function(){
